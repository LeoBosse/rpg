...
class Editor:
	...
	def Display(self, fenetre, (screen_x, screen_y)):
		...
		for l in xrange(1 + int(NB_CELLS_SCREEN_X * self.display_map_ratio)):
	...
	def ManageMouseCLIC(self, (x, y), type):
		...
		if type == "down" and x > NB_PIX_SCREEN_X * self.display_map_ratio:
			...
			for w in xrange(min(w1, w2), max(w1, w2) + 1):
				for c in xrange(min(c1, c2), max(c1, c2) + 1):
